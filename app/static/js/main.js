document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.recruiters && typeof data.recruiters === 'string') {
                try {
                    const recruiters = JSON.parse(data.recruiters);
                    if (recruiters.length > 0) {
                        recruitersContainer.innerHTML = recruiters.map(r => {
                            // Validate URL - must be a valid LinkedIn URL, not null, empty, or placeholder
                            const hasValidUrl = r.url &&
                                typeof r.url === 'string' &&
                                r.url.startsWith('https://www.linkedin.com/in/') &&
                                !r.url.includes('...');
